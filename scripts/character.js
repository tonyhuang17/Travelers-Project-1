...
const renderCharacter = character => {
  ...
  homeworldSpan.innerHTML = `<a href="/content/planet.html?id=${character?.homeworld.id}">${character?.homeworld.name}</a>`;
  const filmsLis = character?.films?.map(film => `<li><a href="/content/film.html?id=${film.id}">${film.title}</li>`)
